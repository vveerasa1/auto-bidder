        ...
        return res
          ...
          .json(
            new ApiResponse(
              404,
              [],
              "No templates found for the specified user."
            )
          );
      }
...
const updateTemplate = asyncHandler(async (req, res) => {
  ...
  if (!templateId || !userId || !category || !content) {
    throw new ApiError(
      400,
      "Template ID, User ID, category, content are all required."
    );
  }
      ...
      const updatedTemplate = await Templates.findByIdAndUpdate(
        templateId,
        {
          $set: {
            ...
            category,
            content,
            skills: skills || [], // Update skills only if provided, otherwise use an empty array
          },
        },
        { new: true } // To return the updated document
      );
...
const addAITemplate = asyncHandler(async (req, res) => {
  ...
  // Validate that all required fields are present
  if (!userId || !content || !wordsCount) {
    message = "User ID, category, content, wordsCount are all required.";
    throw new ApiError(400, message);
  }
      ...
      return res
        ...
        .json(
          new ApiResponse(
            200,
            savedTemplate,
            "AI Template created successfully"
          )
        );
    } catch (error) {
      // Handle errors
      ...
      return res
        ...
        .json(
          new ApiResponse(
            200,
            templates,
            "AI Templates retrieved successfully."
          )
        );
    } catch (error) {
      // Handle errors
...
const updateAITemplate = asyncHandler(async (req, res) => {
  ...
  // Validate that templateId and all required fields are present
  if (!userId || !content || !wordsCount) {
    message = "User ID, wordscount, content, wordsCount are all required.";
    throw new ApiError(400, message);
  }
    ...
    try {
      ...
      const updatedTemplate = await AiTemplates.findByIdAndUpdate(
        templateId,
        {
          ...
          wordsCount,
          content,
          skills: skills || [], // Update skills only if provided
          portfolioLinks: portfolioLinks || [], // Update portfolioLinks only if provided
        },
